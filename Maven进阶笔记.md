...
<dependency>
    ...
    <optional>false</optional> <!--当为true时隐藏-->
</dependency>
```
...
#### 4.3 配置子工程中可选的依赖关系（这配置的东西是不强制要继承的，而是可以选择的）
...
<parent>
    ...
    <version>1.0-SNAPSHOT</version>
    <!--填写父工程的pom文件，根据实际情况填写(不写也能用，辅助用的)-->
    <relativePath>../maven_parent/pom.xml</relativePath>
</parent>
```

#### 4.5 在子工程中配置使用父工程中可选依赖的坐标（不配版本号才用的是父工程继承的）
...
##### ④：配置maven打war包时，忽略web.xml检查（在web的模块中的pom中配置），也可以添加一个web.xml文件里面啥也不写方便一些
...
也可以直接在idea中：![跳过测试](assets\跳过测试.png)
...
上面的版本号可以在为写上面东西时执行 tests  指令后查看（在idea中 可以直接输入maven-surefire-plugin 后按提示给的版本号就行）

![细粒度控制跳过测试](assets\细粒度控制跳过测试.png)
...
各项目组都会有自己的仓库组，这个仓库组中会有若干个宿主仓库，所有的仓库组公用一个代理仓库
