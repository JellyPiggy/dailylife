...
public class CaffeineConfig {
    ...
    public Cache<Long, Item> itemCache(){
        return Caffeine.newBuilder()
                .initialCapacity(100)
                .maximumSize(10_000) //中间下划线只是个分隔符，为了方便读用的
                .build();
    }
...
![image-20210821081826400](assets\image-20210821081826400.png)

Nginx编程需要用到Lua语言，因此我们必须先入门Lua的基本语法。

## 3.1.初识Lua

Lua 是一种轻量小巧的脚本语言，用标准C语言编写并以源代码形式开放， 其设计目的是为了**嵌入应用程序中，从而为应用程序提供灵活的扩展和定制功能**。官网：https://www.lua.org/
...
可以直接在shell控制台中输入lua 然后写lua语句



### 3.2.2.声明变量

Lua声明变量的时候无需指定数据类型，而是**用 local 来声明变量为局部变量**：
...
类似Java的条件控制，例如if、else if、else语法：
...
​	并没有得到想要的结果，直接访问远端http://175.24.191.162:8081/api/item发现正常返回，那只能说明本地的nginx反向代理没起到效果，查看 nginx.conf 配置文件后发现没有问题，想起来是在该 nginx 配置文件前就启动了 nginx，那么就要重启一下，怎么重启呢？先试了restart，没用，又试了下 stop ，但在浏览器还是能正常访问 localhost 说明没有关闭，然后就想到直接在资源管理器里面关，结果还是能访问，我擦，然后就网上搜了关闭nginx的方法，控制台执行：
...
local resp = ngx.location.capture("/path",{
    ...
    body = "c=3&d=4"  -- post方式传参数，get和post可别都写，只可以是一种用这个的话上面请求方式也要改
})
```
...
```lua
-- 引入自定义common工具模块，返回值是common中返回的 _M，这里因为common.lua就在lualib目录下,所以require中直接写common就行了，如果在lualib目录下的子目录中，就得带上子目录名
...
只要id不变，每次hash运算结果也不会变，那**就可以保证同一个商品，一直访问同一个tomcat服务，确保JVM缓存生效。**
...
#### 4）同理，我们的Nginx反向代理时OpenResty也应如此

此处我们的 OpenResty 只有一个，但是等以后集群时有多个，

而默认的负载均衡规则是轮询模式，当我们查询时第一次查 a 时在其中一个 OpenResty  中产生缓存，但下次还查这个又轮询到另一个 OpenResty  ，那上次的缓存就没生效，所以同上面 tomcat 的处理方式一样
...
-- 初始化redis
local red = redis:new()
-- 分别表示建立连接超时时间，发送请求超时时间，响应结果超时时间
...
这里给商品基本信息设置超时时间为30分钟，库存为1分钟（**像这种在实际中最好别往这里面存，因为随时可能变化，而这到期才更新**）。
...
因为 canal 监听到数据库的增删改消息后要把监听到的变化数据转成实体类传过来，那它怎么知道怎么转换呢（mybatis底层自己实现了，但 canal 不依赖于它），所以这里，通过@Id、@Column、等注解完成Item与数据库表字段的映射：**主要是在主键上加@Id，还有不属于表的数据加上@Transient，而@Column只有在字段名称与数据库中列的名称不一致才有必要使用**
