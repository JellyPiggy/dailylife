...
（redis 是无法操作直接物理内存的，操作系统会分配它一个虚拟内存，而后操作系统会维护一个虚拟内存与物理内存之间的映射关系表，这个表就是页表，通过页表就可以实现到物理内存的读写操作）

fork采用的是copy-on-write技术：

- 当主进程执行读操作时，访问共享内存；
- 当主进程执行写操作时，则会拷贝一份数据，执行写操作。（避免脏写）
...
AOF全称为Append Only File（追加文件）。Redis处理的每一个写命令都会记录在AOF文件，可以看做是命令日志文件。(下面$后面的是命令中字符的长度)
...
- slave节点请求增量同步
- master节点判断replid，发现不一致，拒绝增量同步，发送master版本信息。
...
- 主观下线：如果某sentinel节点发现某实例未在规定时间响应(down-after-milliseconds)，则认为该实例**主观下线**。
...
- 然后判断slave节点的slave-priority值(在redis.conf中，默认一样)，越小优先级越高，如果是0则永不参与选举
...
public LettuceClientConfigurationBuilderCustomizer clientConfigurationBuilderCustomizer(){
    //这里用的是 lambda表达式，如果一个接口只有一个方法，那其匿名内部类对象可以这样写
...
数据key不是与节点绑定(如果跟节点绑定的话，那么获取的时候就不知道去哪获取了)，而是与插槽绑定。操作redis中任意key的时候都会有效部分计算插槽值，然后根据插槽值判断连哪一个节点，完成请求的路由
...
如何将同一类数据固定的保存在同一个Redis实例？（查找同一类东西在同一个上就可以避免反复的重定向）
...
4）当7002再次启动（docker restart 对应的容器），就会变为一个slave节点了：
...
- 缺省：默认的流程，如图1~6歩
- force：省略了对offset的一致性校验，**使用场景是 master 节点 down 的情况下的人工故障转移**
